...
\begin{equation}
C.P\:of\:A=\frac{B}{A}=\frac{12526}{256}=\frac{48.9296875}{1}
\end{equation}
...
\textit{Proof}
\begin{equation}
Final\:Price=\frac{B+Buy\:Price - Sell\:Price}{A} = 46.483203125
\end{equation}
Taking (7) \& (11) : $M.P=F.P$ Hence the buy and sell price is found and proved to move the token price to the market price.
...
In centralized Order Books buyers are matched with sellers on the same price rate where in a swap book an order is executed when the sum of all buyers equals sum of all sellers regardless of which token it is. This is a condition in a swap book for it to execute the order to a liquidity Pool


\begin{equation}
\sum_{n=}^{A} x = \sum_{n=}^{B} x
\end{equation}
