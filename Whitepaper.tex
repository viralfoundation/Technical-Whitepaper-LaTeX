...
The live fed to the  is calculated every $x$ minute where the newly updated data of all user's local variables shall not increase the global variable data, promising resistant to users manipulation to change their data \\
...
\item The swap orders will be instantly added to the swap book and sent to users wallet once swapped successfully. For illiquid markets, the swap orders shall sit on the user swap book pending orders, which can be retrived or cancelled at any time. This provides more liquidity to the DEX Pools.
\item The user successfully withdraws multi-token rewards from a single transaction.
\end{itemize}
