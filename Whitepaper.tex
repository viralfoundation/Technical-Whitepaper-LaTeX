...
%\usepackage[a4paper , top=45mm, right=40mm,left=40mm, bottom=45mm]{geometry}
...
\hypersetup{
    ...
    filecolor=magenta,      
    urlcolor=cyan,
    pdftitle={X-Whitepaper Draft 0.1}
}
...
\subsubsection{Chain Integrated Scaling Solutions}
...
An Overview of how Blinkchain's UTXO JSON Object will look like, which will include an oracle value in USD, the elapsed time of the UTXO calculated in negative count (Further explained in Stable Taxes Section) and further calculation done by client to provide output of spend value and tax value which are calculated actively in the backend of the client server. The following data, calculations and terminologies are described in the below topics.

\begin{lstlisting}[caption={Blinkchain UTXO JSON Object Data}, numbers=none]
...
For each transaction, the Oracle value is stamped by a validator or client within the block and the gains tax is analyzed when a particular utxo is spent (transferred) in a later block. The node must comein consensus to the oracle value of the block's transaction in order to keep track of the live oracle data updated per slot (more on this in the  oracle section on the chain).\\
...
The oracle value specifies the real world value of a specific UTXO which can tell the public that a profit has been made and it can be taxed while it's spent. Through the \textit{initial oracle value} and \textit{locked value}, the gains shall be calculated and the tax can also be put forth for the transaction to take place. The values that are used for calculating gains tax are given,\\
...
As the above values are calculated at series to find and verify if the taxes are remitted without evasion, only few are needed to stamped inside a utxo such as \textit{initial oracle value} $O_i$ and \textit{locked value} $V_l$. All the other values are calculated in sequence in an algorithm that follows below,
...
\item These two are stamped and the other values can be actively updated in wallet applccations in real-time to provide real-time balance with remitted taxes, but will only calculated and verified by the nodes only when the utxo is spent.
\item The Oracle Price $O_w$ of every asset accepted as a registered blinkchain tender (More on Token Minter Module) is updated as an On-Chain oracle for every slot (typically 2 mins) that can provide the Current Oracle Price $O_c$ for every UTXO which can update Spend $V_s$ and Tax values $V_t$.
\item 
\end{itemize}
...
Here Spend Value and remaining is tax value
About Spend Value
What if it's above the spend value
Client Side Balance
No Active Update but only while transacting
Tax on Losses not gains
Transaction Construction by Client Wallet Apps
To Note : Transfer Fees, Base Fee, etc not included
When the tax value is not correct, deducted from payers change or rejects the tx
Bitcoin UTXO vs Blinkchain UTXO
Removes non-taxable gifts while selling at exchanges.

\begin{center}
\begin{figure}
\includegraphics[width=\textwidth]{gainstaxutxo}
\caption{Gains Tax on UTXOs during a transaction}
\end{figure}
\end{center}
...
Intro
Only for Stable Tokens represnted
UTXO brings seperate balance added one by one
So UTXOs should be cleaned once every appointed time
UTXO Cleaning and threshold days
Limit reached old UTXO Penalty
Missing UTXOs drawback losing funds
...
\subsection{Proof-of-Delegation}
