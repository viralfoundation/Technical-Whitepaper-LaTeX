...
\item These two are stamped and the other values can be actively updated in wallet applications in real-time to provide real-time balance with remitted taxes, but will only be calculated and verified by the nodes only when the utxo is spent.
...
\[O_t = O_p \times T\%\]
\item E.g., Lets assume that the gains tax are @ 15\% hence the total tax oracle value $O_t$ = $875,000,000,000 \times 15\%$ = 131,250,000,000 USD Winks
...
When the tax value is not correct
\end{comment}
