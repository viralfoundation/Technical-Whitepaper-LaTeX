...
There are various factors a user can opt to get the best price possible. A trader can opt for two models of dex aggregator service provided in the Viral DEX, 1. Automated 2. Manual. In an automated service an algorithm is run to provide the best possible price by splitting up the trades into multiple pool swap books. Automated algorithm for splitting trades will be slower than manual due to different trading volume of opposite token activity in particular swap books. The user can opt for a manual method of selecting a specific pool for a quicker swap\\
...
The factors determining the best possible price
\begin{enumerate}[leftmargin=+0.2in]
