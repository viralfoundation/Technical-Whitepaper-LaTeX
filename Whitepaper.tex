...
\textbf{VDEX - Viral Decentalized Exchange}\\

Viral DEX is Viral Smart Chain's native low-fee automated market maker based decentralized exchange that facilitates trade between users to exchange cryptocurrencies. VDEX will be featuring initial decentralized trades between available tokens on the smart chains such as Viral Coin, Viral Stable Coin and other wrapped tokens such as vBTC, vETH, vSOL, etc using Liquidity Pools and Constant Product Maker algorithm to determine prices of the exchange pairs. \\

\textbf{Liquidity Pools}\\

Viral DEX consists of different liquidity pools for different trading pairs. Each trading pair will consist of two cryptocurrencies/tokens e.g VRL-VSC, VSC-vBTC. Every liquidity pool is a smart contract on the Viral Smart Chains which holds the reserves of the two tokens of the trading pairs which will allow anyone to deposit and withdraw funds by providing one of the trading token with a specific set of smart contract rules. The current prices of trading pairs will be determined by the constant product formula\\
\begin{equation}
x*y=k
\end{equation}
Here x and y denotes the reserves of two tokens of the trading pair. This constant product formula dictates that the $k$ (constant) should maintain the same. So, inorder for users to withdraw some funds from the reserve they should provide the exact number of alternate tokens along with the dex fee to maintain the constant $k$ to the liquidity pool/smart contract.
...
\textbf{Viral Coin and Viral Stable Coin}

\textbf{Minimizing Price Impacts}
