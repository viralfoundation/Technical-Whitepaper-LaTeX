...
The oracle value stamped can be called as the \textit{initial oracle value} as it represents the oracle value of the particular utxo's asset balance taken from the slot it's created (Epochs, Slots and Packets is explained in the Block Architecture). The United States Dollar (USD) is currently suitable for most of the nodes to collaborate and provide oracle data, so the \textit{initial oracle value} is stamped in a smallest denomination of USD ($10^{-8}$) which is comparable to the smallest denomination ($10^{-8}$) of all the assets circulating in the Blinkchain.\\

\begin{center}
$1$ USD = $1 * 10^{-8}$ Oracles = $100,000,000$ Oracles
\end{center}
