...
\begin{enumerate}
\item Country (E.g., USA, India, Germany)
\item State (E.g., Florida, Hamburg, Tamil Nadu)
\item Type
	\begin{enumerate}
	\item Personal
	\item Merchant
	\item Central Government
	\item State Government
	\item Non-Profit
	\end{enumerate}
\end{enumerate}

There are 5 individual categories in wallet types that are common for all the countries and states. Each On-Chain wallet is identifiable by it's country, state and type. States are determined by first-level administrative divisions, whereas second and further levels cannot be classified as it increases the difficulty to generate vanity addresses as the prefix patterns exponentially increases the computation needed.  

\subsubsection{White-listed Vanity Addresses}

Vanity pattern with 3 digit prefix can produce 195112 combinations in which each combination white-listed. To identify every wallet by it's country,state and type we would combinations to be $< 195112$
