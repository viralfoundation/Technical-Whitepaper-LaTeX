...
\begin{itemize}[leftmargin=+0.2in]
\item Every Pool will have strict time period after which it can able to fetch the market price and update the buy/sell price
\item The sucessfully rebased pools will be locked/freezed until all the pools update it's buy/sell price for every rebase group (A rebase group is calculated when all the pools rebases wholly, typically every time period of the largest pool)
\item For every new pool the time period of all the pools will be updated accordingly
\item The Whole rebase time will increase by 2 times after every new pool is sharded
\end{itemize}
